  ...
  ninja.fn.extend({
    ...
    button: function (options) {
      ...
      var $button = ninja('<span/>', {
        className: 'ninja ninjaBorder ninjaButton ninjaInline ninjaShadow ninjaUnselectable'
      });
      if (options.gradient) {
    ...
    icons: {alert: '!', arrowDown: 'v', arrowLeft: '<', arrowRight: '>', arrowUp: '^', calendar: 'c', check: '√', chrome: 'C', circle: '•', clear: 'x', clip: '©', diamond: '◊', directions: 'd', edit: 'e', facebook: 'f', female: ']', firefox: 'F', flag: 'ƒ', gear: 'g', heart: '¼', heartPlus: '½', heartMinus: '¾', help: '?', home: 'π', infinity: '∞', info: 'i', install: 'I', link: '±', linux: 'L', lock: 'l', mac: 'M', mail: '@', male: '[', map: 'm', moveBack: '‹', moveDown: 'ˇ', moveForward: '›', moveUp: 'ˆ', minus: '-', ninja: 'N', not: 'ø', options: '‡', photo: 'P', plus: '+', remove: 'X', safari: 'S', search: 'Q', shipping: 's', star: '*', thought: 'T', thumbsDown: '≤', thumbsUp: '≥', time: 'Ø', triangle: '∆', tweet: 't', unlock: 'u', user: '(', users: ')', video: 'V', watch: 'o', weather: 'w', windows: 'W'},
        ...
        'update.ninja': function (event) {
          ...
          if (options.button) {
            var
              $icon = ninja().icon({
                name: 'clear'
              }).addClass('ninjaPopupButtonForeground'),
              $iconBackground = ninja().icon({
                name: 'circle'
              }).addClass('ninjaPopupButtonBackground'),
              $button = ninja('<span/>', {
                className: 'ninjaPopupButton'
              }).append($iconBackground, $icon).click(function () {
                $popup.remove();
              });
            $popup.append($button);
          }
    ...
    tabs: function (options) {
      ...
      $.each(options.choices, function (i, choice) {
        var $choice = $('<span/>', {
          className: 'ninja ninjaBorder ninjaGradient ninjaInline ninjaShadow ninjaTab ninjaUnselectable',
          html: choice.html || choice
        }).bind({
          'click.ninja': function () {
