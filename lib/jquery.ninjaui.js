  ...
  ninja.fn.extend({
    ...
    */
    list: function (options) {
      ...
      }).bind('keyup.ninja', function (event) {
    ...
    */
    suggest: function (options) {
      ...
      var $clear = $.ninja().icon('xAlt').addClass('ninjaSuggestClear');
      $input.bind('focus.ninja', function () {
        if ($input.val() === '') {
          $clear.remove();
        }
        else {
          $suggest.append($clear.bind('click.ninja', function () {
            $input.val('');
            $clear.remove();
          }));
//            var choice = choice.toString().replace(new RegExp(options.value, 'gi'), '<strong>' + options.value + '</strong>');
        }
      });
