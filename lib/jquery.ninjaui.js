  ...
  ninja.fn.extend({
    ...
    create: function (kuji, customOptions) {
      ...
      return this.each(function (i, object) {
        ...
        if (options.kuji === 'button') {
          $(object).addClass('border button inline unselectable').css({
          ...
          $(object)
            .removeAttr('id')
            .addClass('border drawer editable')
            .css({
              width: options.width
            })
          ...
          $(object)
            .addClass('ninja border editable list shadow unselectable')
            .hide()
            .ninja().round()
            .css({
          ...
          $(object)
            .addClass('ninja border editable inline panel')
            .ninja().round();
          if (options.icon || options.title) {
            var bar = $('<div/>', {
              className: 'ninja bar button enabled gradient unselectable'
            })
          ...
          var hover = function (selected) {
            $('.ninja.icon', object).each(function (i, icon) {
              if (selected + 1 > i) {
                $(icon).addClass('selected').removeClass('group');
              } else if ($.inArray(options.selectedGroup, options.values) + 1 > i) {
                $(icon).addClass('selected group');
              } else {
                $(icon).removeClass('selected group');
              }
            });
          };
          $('.ninja.icon', object).each(function (i, icon) {
          ...
          options.button = $('<span/>', {
            className: 'ninja border enabled button gradient inline'
          }).ninja().round({ radius: '0.8em' });
          options.level = $('<div/>', {
            className: 'ninja enabled gradient level'
          ...
          var groove = $('<div/>', {
            className: 'ninja border enabled gradient selected groove'
          ...
          options.input = $('<input/>', {
            ...
            className: 'ninja border editable query',
            type: 'text',
            ...
            .blur(function () {
              $(options.list).ninja().disable();
              if ($(this).val() === '') {
                $(this).addClass('editable').val(options.value || options.title);
              }
          ...
          $(object).addClass('ninja suggest').append($(options.input), $(options.list));
          if (options.width) {
            $(object).width(options.width);
          } else if ($(object).is('span')) {
            $(object).addClass('inline');
          }
        } else if (options.kuji === 'window') {
          var window = $('<span/>', {
            className: 'ninja window'
          });
    ...
    deselect: function (customOptions) {
      return this.each(function (i, object) {
        ...
        if (options.isSelected) {
          $(object).addClass('enabled').removeClass('selected');
          if (options.icon) {
        ...
        if (!options.isEnabled) {
          if (options.kuji === 'list') {
            var move = function (direction) {
              var choices = $('.ninja.choice', object).removeClass('enabled gradient'),
              ...
              options.value = options.values[slot];
              $(choices[slot]).addClass('enabled gradient');
            ...
            $(object).addClass('enabled').removeClass('disabled').animate({ opacity: 1 });
            if ($(object).is('input')) {
    ...
    round: function (customOptions) {
      return this.each(function (i, object) {
        ...
        if (options.radius) {
          ...
          } else if (style.MsBorderRadius !== undefined) {
            borderRadius = '-ms-border';
          } else if (style.MozBorderRadius !== undefined) {
            borderRadius = '-moz-border';
          } else if (style.OBorderRadius !== undefined) {
            borderRadius = '-o-border';
          }
          if (borderRadius && borderRadius === 'border') {
            ...
            $(object).css(borderRadius + '-top-left-radius', radii[0])
              .css(borderRadius + '-top-right-radius', radii[1])
              .css(borderRadius + '-bottom-right-radius', radii[2])
              .css(borderRadius + '-bottom-left-radius', radii[3]);
          }
        }
      });
    },
      ...
      return this.each(function (i, object) {
        ...
        if (options.kuji === 'icon') {
          ...
          if ($.inArray(options.icon, ['add', 'arrange', 'beverage', 'bookmark', 'caution', 'check', 'down', 'download', 'edit', 'flag', 'food', 'gear', 'group', 'heart', 'home', 'in', 'left', 'lock', 'mail', 'no', 'out', 'phone', 'photo', 'print', 'profile', 'question', 'refresh', 'remove', 'right', 'search', 'star', 'target', 'unlock', 'up', 'upload', 'video']) > -1) {
            $(object).addClass(options.icon).css({
              minHeight: '16px',
              minWidth: '16px'
            });
          ...
          if (options.values[0]) {
            ...
            $.each(options.values, function (i, value) {
              var choice = $('<div/>', {
                ...
                .mouseover(function () {
                  $(this).addClass('enabled gradient');
                })
                ...
                .mouseleave(function () {
                  $(this).removeClass('enabled gradient');
                });
              if (options.names) {
    ...
    wait: function (customOptions) {
      return this.each(function (i, object) {
        ...
        }).ninja().round().text(options.message).prepend($('<span/>', {
