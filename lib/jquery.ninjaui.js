...
(function ($) {
  ...
  */
  ...
  ninja.fn.extend({
    ...
    /* Bubble - container layer, positioned under calling object, dismissed via ninja().pop() or the escape key.
      pop - adds a button (x) to dismiss.
      radius - custom radius for the bubble, see ninja().radius().
      theme - custom theme for the bubble, see ninja().themeToggle().
      window - center bubble within browser window
    */
    ...
    choices: function (options) {
      return this.each(function () {
        var $object = $(this);
        $.each(options.values, function (value) {
          $object.append(value);
        });
      });
    },
    ...
    capitalize: function (name) {
      return name.replace(/^\w/, function (initial) {
