    ...
    suggest: function (options) {
      ...
      $input.bind({
        ...
        'blur.ninja': function (event) {
          ...
          if ($('.ninjaButton.ninjaHovered', $bubble).length === 0) {
            $bubble.detach();
          }
        },
        'keydown.ninja': function (event) {
        ...
        'update.ninja': function (event) {
          ...
          var $list = $.ninja().list({
            choices: $.map(event.choices, function (choice) {
              choice.display = choice.html.replace(new RegExp(value, 'gi'), '<strong>' + value + '</strong>');
              choice.html = choice.html;
              choice.select = function (event) {
                $suggest.trigger({
                  type: 'select.ninja',
                  html: event.html
                });
    ...
    */
    update: function (options) {
      return this.each(function () {
        ...
        else if ($object.is('.ninjaSuggest')) {
          $object.trigger({
            type: 'update.ninja',
