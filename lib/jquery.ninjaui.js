    ...
    create: function (kuji, customOptions) {
      ...
      return this.each(function (i, object) {
        ...
        if (options.kuji === 'button') {
          ...
          }).ninja().round().css(options.colors.selected).width(options.slot * options.increment);
          ...
          options.input = $('<input/>', {
            ...
            .blur(function () {
              $(options.list).ninja().disable();
              if ($(this).val() === '') {
                $(this).css(options.colors.editable).val(options.value || options.title);
              }
        ...
        if (!options.isEnabled) {
          if (options.kuji === 'list') {
            var move = function (direction) {
              var choices = $('.ninja.choice', object).removeClass('gradient').css({
                backgroundColor: options.colors.editable.backgroundColor,
                borderColor: options.colors.enabled.borderColor,
                color: options.colors.enabled.color
            ...
            $(object).removeClass('disabled').addClass('enabled').animate({ opacity: 1 });
            if ($(object).is('input')) {
