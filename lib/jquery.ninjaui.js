      ...
      return this.each(function (i, object) {
        ...
        // $('span').ninja().create('slider');
          ...
          if (navigator.userAgent.toLowerCase().match(/(iphone|ipod|ipad)/)) {
            ...
            $(select).val(options.selected).width(options.width).change(function () {
            ...
            var buttonDiameter = $(options.button).outerWidth();
            var buttonRadius = Math.round(buttonDiameter / 2);
            var trackWidth = (options.increment * slots) + buttonDiameter;
            var groove = $('<div/>', {
              className: 'ninja border gradient selected groove'
            }).ninja().round().css(options.colors.enabled).click(function (event) {
              var slot = function () {
                var max = slots - 1;
                var x = Math.round((event.pageX - buttonDiameter) / options.increment);
                if (x < 0) {
                  return 0;
                }
                else if (x > max) {
            ...
            console.error(trackWidth);
            $(track).width(trackWidth).prepend($(groove));
            $(options.status).width(trackWidth);
            ...
            $(options.button).mousedown(function (event) {
              ...
              $(document).mouseup(function (event) {
                ...
                var slot = function () {
                  var x = Math.round((event.pageX - offsetX - buttonDiameter) / options.increment);
                  if (x < 0) {
                    return 0;
                  }
                  else if (x > slots) {
