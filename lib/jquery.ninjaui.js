  ...
  */
  ...
  ninja.fn.extend({
    ...
    */
    ...
    /* Icon - ninja object that displays a semi-transparent black or white symbol, depending on the brightness of its parent's background.
      ninja().icon({
        name: 'target' | 'search' | 'etc' - custom symbol for the ninja icon.
      })
    */
    ...
    /* Icon refresh - changes the icon's color to black or white, depending on the brightness of its parent's background.
    */
    ...
    disable: function (options) {
      return this.each(function () {
        ...
        if ($.isFunction(options)) {
          ...
          if ($object.is('.ninjaButton')) {
            $object.removeClass('ninjaEnabled').animate({
              opacity: 0.8
            });
      ...
      return this.each(function () {
        ...
        if ($.isFunction(options)) {
          ...
          if (!$object.is('.ninjaEnabled')) {
            $object.addClass('ninjaEnabled').animate({
              opacity: 1
            });
            $object.bind({
    ...
    list: function (options) {
      ...
      $.each(options.choices, function (i, choice) {
        ...
        if (choice.spacer) {
          ...
          $choice.append($.ninja().button({
            gradient: false,
            html: choice.html || choice
          }).select(function () {
            /* individual select function */
            if (choice.select) {
              choice.select($(choice.html));
            }
            /* global select function */
            if (options.select) {
              options.select($(choice.html));
            }
            $(this).ninja().deselect();
          }));
        }
