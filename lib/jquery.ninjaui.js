...
(function ($) {
  ...
  ninja.fn.extend({
    ...
    /* Pop-Up - container layer object, positioned under calling object, dismissed via detach(), remove() or the escape key.
      ninja().popup({
        button: true - adds a button (x) to dismiss.
        radius: '0.5em' custom radius for the popup, see ninja().radius().
        theme: 'light' | 'dark' | 'etc' - custom theme for the popup, see ninja().themeToggle().
        window: true - center ninja popup within browser window
      })
    */
      ...
      $popup.bind({
        'html.ninja': function (event) {
          ...
          if (options.window) {
            ...
            var $stem = $.ninja().icon('arrowUp').addClass('ninjaPopupStem').css({
