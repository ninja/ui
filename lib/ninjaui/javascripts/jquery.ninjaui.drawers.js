...
$.fn.ninjaDrawersClose = function(callback){
  return this.each(function(i, drawer){
    ...
    $(drawer).slideUp(function(){
      ...
      $(icon).addClass('ninjaIcon-right').removeClass('ninjaIcon-down').ninjaIconLightness();
...
$.fn.ninjaDrawersOpen = function(callback){
  return this.each(function(i, drawer) {
    var options = $(drawer).data().options;
    ...
    $(icon).addClass('ninjaIcon-down').removeClass('ninjaIcon-right').ninjaIconLightness();
    $(drawer).slideDown();
    if (callback)
      callback(drawer);
    if($(handle).hasClass('ninjaLast')){
