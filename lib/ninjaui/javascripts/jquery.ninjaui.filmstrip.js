$.fn.ninjaFilmstripCreate = function(customOptions) {
  ...
  var bar = $(filmstrip).prepend('<div class="ninjaFilmstripBar"/>').find('.ninjaFilmstripBar');
  if(options.icon || options.title){
    $(bar).append('<span class="ninjaFilmstripTitle"/>');
    $('.ninjaFilmstripTitle', bar).ninjaIconCreate({icon:options.icon, title:options.title});
  };
