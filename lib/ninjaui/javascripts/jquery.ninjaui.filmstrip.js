$.extend($.fn.ninjaui, {
  filmstripCreate:function(customOptions) {
    ...
    global.selected.each(function (i, origin) {
      var frames = $(origin).children('span').each(function (i, frame) {
      ...
      filmstrip = $(view).addClass('ninjauiFilmstripView').wrap('<div/>').parent();
      var bar = $(filmstrip).addClass('ninjauiFilmstrip').prepend('<div/>').find('> div').text(options.title || '');
