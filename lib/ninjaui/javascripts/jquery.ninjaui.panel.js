$.fn.ninjaPanelCreate = function(customOptions) {
  var options = $.extend({
    colors:$.ninjaColors,
    icon:null,
    radius:$.ninjaRadius,
    title:null
  }, customOptions);
  return this.each(function(i, panel) {
    $(panel).addClass('ninjaPanel').css(options.colors.background).ninjaRadius({radius:options.radius}).prepend('<div class="ninjaPanelBar"/>');
    var bar = $('.ninjaPanelBar', panel).css(options.colors.foreground).ninjaRadius({corners:'top', radius:options.radius});
    if(options.title){
      $(bar).text(options.title);
    };
    if(options.icon){
      if(options.title){
        $(bar).prepend('&#160;');
      };
      $(bar).prepend('<span class="ninjaPanelIcon"/>');
      $('.ninjaPanelIcon', panel).ninjaIconCreate(options.icon).css({height:$(button).height(), position:'relative', top:'-2px'});
    };
  });
};
